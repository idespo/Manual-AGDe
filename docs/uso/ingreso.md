# Formas de ingresar al sistema AGDe

1. **Acceso directo por enlace web**

    Ingrese al sistema mediante el siguiente enlace:
    [https://agd.una.ac.cr/share/page/](https://agd.una.ac.cr/share.page/){:target="_blank"}

2. **A través del sitio web institucional**

    Puede acceder al sistema siguiendo esta ruta desde la página principal de la UNA:

      1. Sitio web institucional: [https://www.una.ac.cr/](https://www.una.ac.cr/){:target="_blank"}
      2. Sección: Información para funcionarios
      3. Subapartado: Archivo y Gestión de Documentos Electronicos (AGDe)

3. **Desde notificaciones por correo electrónico**

    La persona usuaria recibira un correo electrónico como el que se ve en la siguiente imagen:
    
    ![Ingreso al AGDe via correo electrónico](/images/img_ingreso_email_001.png)